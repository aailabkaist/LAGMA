...
class NearestEmbedFunc(Function):
    ...
    def forward(ctx, input, emb):
        ...
        # find nearest neighbors check -----------
        # aa = x_expanded.squeeze(-1)[0,:]
        # a_exp = aa.unsqueeze(1) 
        # bb = a_exp - emb_expanded
        # dist0 = torch.norm(bb, 2, 0)

        # dist_com = []
        # for i in range(ctx.num_emb):
        #     dist_com.append( torch.norm(aa - emb_expanded[:,i],2,0) )        
        # dist_com = torch.stack( dist_com, dim=0)
        #------------------------------------------
...
class NearestEmbed(nn.Module):
    def __init__(self, num_embeddings, embeddings_dim):
        ...
        self.weight = nn.Parameter(torch.rand(embeddings_dim, num_embeddings)) # emb~U[0,1)
        #self.weight = nn.Parameter(2*torch.rand(embeddings_dim, num_embeddings)-1) # emb~U[-1,1)
