...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        # Qatten
        self.unit_dim = 4 + self.shield_bits_ally + self.unit_type_bits # dist,relposx,relposy,health + @
    ...
    def step(self, actions):
        ...
        info = {"battle_won": False} # to prevent output error when there is protocol error
        ...
        #self.reward = reward # discarded in recent works
    ...
    def get_obs_size(self):
        ...
        own_feats = self.get_obs_own_feats_size() # 5: unit_type(3), health, shield
        move_feats = self.get_obs_move_feats_size() # 4

        n_enemies, n_enemy_feats = self.get_obs_enemy_feats_size() # 4 + 5 (unit_type, health, shield)
    ...
    def init_units(self):
        ...
        while True:
            ...
            for i in range(len(enemy_units_sorted)):
                enemy_unit = enemy_units_sorted[i]
                self.enemies[i] = enemy_unit
                if self._episode_count == 0:
                    self.max_reward += enemy_unit.health_max + enemy_unit.shield_max
            # ######## WTH ##############
    ...
    def get_structured_state(self):
        """Returns the structured global state, for visualization
        """
        info_list = ['x', 'y', 'cd', 'cd_per', 'health', 'health_max']

        if self.shield_bits_ally > 0:
            ally_info_list = info_list + ['shield', 'shield_max']
        else:
            ally_info_list = info_list

        if self.shield_bits_enemy > 0:
            enemy_info_list = info_list + ['shield', 'shield_max']
        else:
            enemy_info_list = info_list

        ally_info = {key: [] for key in ally_info_list}
        enemy_info = {key: [] for key in enemy_info_list}

        # center_x = self.map_x / 2
        # center_y = self.map_y / 2

        for al_id, al_unit in self.agents.items():
            ally_info['x'].append(al_unit.pos.x)
            ally_info['y'].append(al_unit.pos.y)
            # ally_info['max_cd'].append(self.unit_max_cooldown(al_unit))
            ally_info['health'].append(al_unit.health)
            ally_info['health_max'].append(al_unit.health_max)

            if (
                self.map_type == "MMM"
                and al_unit.unit_type == self.medivac_id
            ):
                ally_info['cd'].append(al_unit.energy)  # energy
                ally_info['cd_per'].append(al_unit.energy / self.unit_max_cooldown(al_unit))
            else:
                ally_info['cd'].append(al_unit.weapon_cooldown)  # cooldown
                ally_info['cd_per'].append(al_unit.weapon_cooldown / self.unit_max_cooldown(al_unit))

            if self.shield_bits_ally > 0:
                ally_info['shield_max'].append(self.unit_max_shield(al_unit))
                ally_info['shield'].append(al_unit.shield)  # shield

        for e_id, e_unit in self.enemies.items():
            enemy_info['x'].append(e_unit.pos.x)
            enemy_info['y'].append(e_unit.pos.y)
            enemy_info['health'].append(e_unit.health)
            enemy_info['health_max'].append(e_unit.health_max)

            if self.shield_bits_enemy > 0:
                enemy_info['shield_max'].append(self.unit_max_shield(e_unit))
                enemy_info['shield'].append(e_unit.shield)

        ally_info = {key: np.array(value) for key, value in ally_info.items()}
